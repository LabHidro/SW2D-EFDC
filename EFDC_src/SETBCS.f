...
C  
      DO LL=1,NPBW  
        ...
        IF(ISPBW(LL).LE.1) THEN  
          SVB(LEAST(L))=0.  
          SWB(LEAST(L))=0.  
          SCAX(LEAST(L))=0.  ! *** Used for On/Off of Coriolis & Curvature Stresses  
        END IF  
        SAAX(LEAST(L))=0.    ! *** Used for On/Off of Horizontal Momentum Stresses (X Dir)
        SAAY(LEAST(L))=0.    ! *** Used for On/Off of Horizontal Momentum Stresses (Y Dir) PMC-Added
        !SDX(LEAST(L))=0.    ! *** Used for On/Off of Horizontal Momentum Diffusion Stresses PMC-Disabled
      ENDDO  
      DO LL=1,NPBE  
        I=IPBE(LL)  
        J=JPBE(LL)  
...
C *** SET THE VELOCITY AVERAGING FACTORS
      ...
      DO LL=1,NQSIJ  
        ...
        ENDIF
        IF(L.LT.LA-2)THEN
          IF(SUB(L).LT.0.5.AND.(SUB(LEAST(L)).GT.0.5.AND.SUB(L+2).GT.0.5))
     &                                                             THEN
            LBERC(NBCS)=LEAST(L)
            SAAX(LBERC(NBCS))=0.
            SAAY(LBERC(NBCS))=0.  
        ...
        IF(L.GT.2.AND.L.LT.LA)THEN
          IF((SUB(L  ).GT.0.5.AND.SUB(LEAST(L)).LT.0.5).AND.
     &     (SUB(LWEST(L)).GT.0.5.AND.SUB(L-2).GT.0.5))THEN
            LBERC(NBCS)=LWEST(L)
            SAAX(LBERC(NBCS))=0.            
            SAAY(LBERC(NBCS))=0.  
            SAAX(L)=0.  
     ...
     &     (SVB(LSC(L)).GT.0.5.AND.SVB(LSC(LSC(L))).GT.0.5))THEN
      ...
      DO NCTL=1,NQCTL  
        ...
        IF(SUB(L).LT.0.5)THEN
          ...
          SAAY(L)=0.  ! PMC
        ENDIF
c        IF(SUB(L).LT.0.5.AND.(SUB(LEAST(L)).GT.0.5.AND.SUB(L+2).GT.0.5))THEN
c          LBERC(NBCS)=LEAST(L)
c          SAAX(LBERC(NBCS))=0.
c          SAAY(LBERC(NBCS))=0.  
c        ENDIF
...
C  
      ...
      DO L=2,LA  
        LS=LSC(L)  
        HUDRY(L)=HDRY+0.5*ABS(BELV(L)-BELV(LWEST(L)))  
        HVDRY(L)=HDRY+0.5*ABS(BELV(L)-BELV(LS))  
        HUWET(L)=HWET+0.5*ABS(BELV(L)-BELV(LWEST(L)))  
...
C  
      IF(DEBUG)THEN
        ...
        DO L=2,LA  
          WRITE(1,1001)IL(L),JL(L),SUB(L),SUB(LEAST(L)),SVB(L),SVB(LNC(L)),  
     &      SPB(L)  
        ENDDO  
        CLOSE(1)  
      ENDIF
